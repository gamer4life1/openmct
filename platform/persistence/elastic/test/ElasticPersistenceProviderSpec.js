...
define(["../src/ElasticPersistenceProvider"], function (
  ...
  describe("The ElasticSearch persistence provider", function () {
    ...
    it("allows object creation", function () {
      ...
      mockHttp.and.returnValue(
        mockPromise({ data: { _id: "abc", _seq_no: 1, _primary_term: 1 } })
      );
    ...
    it("allows object update", function () {
      ...
      mockHttp.and.returnValue(
        mockPromise({ data: { _id: "abc", _seq_no: 1, _source: {} } })
      );
    ...
    it("handles rejection due to _seq_no", function () {
      ...
      // Now perform an update
      mockHttp.and.returnValue(
        mockPromise({ data: { status: 409, error: "Revision error..." } })
      );
      provider
        .updateObject("testSpace", "abc", model)
    ...
    it("handles rejection due to unknown reasons", function () {
      ...
      // Now perform an update
      mockHttp.and.returnValue(
        mockPromise({ data: { status: 410, error: "Revision error..." } })
      );
      provider
        .updateObject("testSpace", "abc", model)
        .then(capture, mockErrorCallback);
