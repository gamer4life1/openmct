...
define(
    ...
    function(ElasticPersistenceProvider) {
      describe("The ElasticSearch persistence provider", function() {
        var mockHttp, mockQ, testSpace = "testSpace", testRoot = "/test",
                             testPath = "db", capture, provider;

        function mockPromise(value) {
          return (value || {}).then ? value : {
            then : function(callback) { return mockPromise(callback(value)); }
          };
        }
        ...
        beforeEach(function() {
          ...
          mockQ.reject.and.callFake(function(value) {
            return {
              then : function(ignored,
                              callback) { return mockPromise(callback(value)); }
            };
          });
          ...
          provider = new ElasticPersistenceProvider(mockHttp, mockQ, testSpace,
                                                    testRoot, testPath);
        });
        ...
        it("allows object update", function() {
          ...
          mockHttp.and.returnValue(
              mockPromise({data : {"_id" : "abc", "_source" : {}}}));
        ...
        it("returns undefined when objects are not found", function() {
          // Act like a 404
          mockHttp.and.returnValue(
              {then : function(success, fail) { return mockPromise(fail()); }});
          provider.readObject("testSpace", "abc").then(capture);
        ...
        it("handles rejection due to _seq_no", function() {
          ...
          provider.updateObject("testSpace", "abc", model)
              .then(capture, mockErrorCallback);
