      ...
      return function install(openmct) {
        ...
        function navigateToPath(path, currentViewKey) {
          ...
          return pathToObjects(path).then((objects) => {
            ...
            unobserve = this.openmct.objects.observe(
            ...
            document.title = browseObject.name; // change document title to
                                                // current object in main view
            ...
            if (defaultProvider) {
              openmct.router.updateParams({view : defaultProvider.key});
            } else {
              openmct.router.updateParams({view : undefined});
              openmct.layout.$refs.browseObject.clear();
            }
          });
        }
        ...
        function pathToObjects(path) {
          return Promise.all(path.map(
              (keyString) => { return openmct.objects.get(keyString); }));
        }

        function navigateToFirstChildOfRoot() {
          openmct.objects.get('ROOT').then(rootObject => {
            openmct.composition.get(rootObject).load().then(children => {
              ...
              if (!lastChild) {
                ...
                let lastChildId =
                    openmct.objects.makeKeyString(lastChild.identifier);
