...
export const getHistoricLinkInFixedMode =
    (openmct, bounds, historicLink) => {
...
export const getNotebookDefaultEntries =
    (notebookStorage, domainObject) => {
...
export const createNewEmbed =
    (snapshotMeta, snapshot = '') => {
      ...
      const date = Date.now();
      const historicLink =
          link ? getHistoricLinkInFixedMode(openmct, bounds, link)
...
export const addNotebookEntry =
    (openmct, domainObject, notebookStorage, embed = null) => {
      ...
      const defaultEntries =
          getNotebookDefaultEntries(notebookStorage, domainObject);
      const id = `entry-${date}`;
      defaultEntries.push({id, createdOn : date, text : '', embeds});
...
export const getNotebookEntries =
    (domainObject, selectedSection, selectedPage) => {
...
export const getEntryPosById =
    (entryId, domainObject, selectedSection, selectedPage) => {
      if (!domainObject || !selectedSection || !selectedPage) {
        return;
      }

      const entries =
          getNotebookEntries(domainObject, selectedSection, selectedPage);
      let foundId = -1;
...
export const deleteNotebookEntries =
    (openmct, domainObject, selectedSection, selectedPage) => {
