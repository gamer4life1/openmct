...
export default class Snapshot {
  ...
  capture(snapshotMeta, notebookType, domElement) {
    this.exportImageService
        .exportPNGtoSRC(domElement, 's-status-taking-snapshot')
        .then(function(blob) {
  ...
  _saveToDefaultNoteBook(embed) {
    ...
    this.openmct.objects.get(notebookStorage.notebookMeta.identifier)
        .then(domainObject => {
          ...
          const defaultPath = `${domainObject.name} > ${
              notebookStorage.section.name} > ${notebookStorage.page.name}`;
  ...
  _showNotification(msg) { this.openmct.notifications.info(msg); }
}
