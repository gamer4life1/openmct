...
define([ 'lodash' ], function(_) {
  function DisplayLayoutToolbar(openmct) {
    return {
      name: "Display Layout Toolbar", key: "layout",
          ...
          forSelection: function(selection) {
            ...
            // Apply the layout toolbar if the selected object is inside a
            // layout, or the main layout is selected.
            return (selectedParent && selectedParent.context.item &&
            ...
            function getUserInput(form) {
              return openmct.$injector.get('dialogService')
                  .getUserInput(form, {});
            }
              ...
              if (selection.length === 1) {
                ...
                return {
                  ...
                  method : function(option) {
                    ...
                    if (form) {
                      getUserInput(form).then(
                          element => selectionPath[0].context.addElement(
                              name, element));
                    } else {
                  ...
                  label : "Add",
                  options : [
                    {"name" : "Box", "class" : "icon-box-round-corners"},
                    {"name" : "Line", "class" : "icon-line-horz"},
                    {"name" : "Text", "class" : "icon-font"},
                    {"name" : "Image", "class" : "icon-image"}
                  ]
                };
              }
            }
              ...
              return {
                ...
                method : function() {
                  ...
                  let prompt = openmct.overlays.dialog({
                    iconClass : 'alert',
                    message :
                        `Warning! This action will remove this item from the Display Layout. Do you want to continue?`,
                    buttons : [
                      {
                        label : 'Ok',
                        emphasis : 'true',
                      ...
                      {
                        label : 'Cancel',
                        callback : function() { prompt.dismiss(); }
                      }
                    ]
                  });
                }
            ...
            function getStackOrder(selectedParent, selectionPath) {
              return {
                ...
                options : [
                  ...
                  {name : "Move Up", value : "up", class : "icon-arrow-up"}, {
                    name : "Move Down",
                    value : "down",
                ...
                method : function(option) {
                  selectionPath[1].context.orderItem(
                      option.value, getAllTypes(selectedObjects));
                }
              };
            }
            ...
            function getXInput(selectedParent, selection) {
              if (selection.length === 1) {
                return {
                  ...
                  property : function(
                      selectionPath) { return getPath(selectionPath) + ".x"; },
                  label : "X:",
                  title : "X position"
                };
              }
            ...
            function getYInput(selectedParent, selection) {
              if (selection.length === 1) {
                return {
                  ...
                  applicableSelectedItems : getAllTypes(selection),
                  property : function(
                      selectionPath) { return getPath(selectionPath) + ".y"; },
                  label : "Y:",
                  title : "Y position"
                };
              }
              ...
              if (selection.length === 1) {
                return {
                  ...
                  applicableSelectedItems : selection.filter(selectionPath => {
                    return selectionPath[0].context.layoutItem.type ===
                           'line-view';
                  }),
                  property : function(
                      selectionPath) { return getPath(selectionPath) + ".x2"; },
                  label : "X2:",
                  title : "X2 position"
                };
              }
            ...
            function getY2Input(selectedParent, selection) {
              if (selection.length === 1) {
                return {
                  ...
                  property : function(
                      selectionPath) { return getPath(selectionPath) + ".y2"; },
                  label : "Y2:",
                  title : "Y2 position"
                };
              }
            ...
            function getTextSizeMenu(selectedParent, selection) {
              const TEXT_SIZE = [
                8, 9, 10, 11, 12, 13, 14, 15, 16, 20, 24, 30, 36, 48, 72, 96,
                128
              ];
              return {
                control : "select-menu",
                ...
                property : function(
                    selectionPath) { return getPath(selectionPath) + ".size"; },
                title : "Set text size",
                options :
                    TEXT_SIZE.map(size => { return {value : size + "px"}; })
              };
            }
            ...
            function getTextButton(selectedParent, selection) {
              return {
                ...
                property : function(
                    selectionPath) { return getPath(selectionPath); },
                icon : "icon-font",
            ...
            function getTelemetryValueMenu(selectionPath, selection) {
              if (selection.length === 1) {
                return {
                  ...
                  property : function(
                      path) { return getPath(path) + ".value"; },
                  title : "Set value",
            ...
            function getDisplayModeMenu(selectedParent, selection) {
              if (selection.length === 1) {
                return {
                  ...
                  options : [
                    {name : 'Label + Value', value : 'all'},
                    {name : "Label only", value : "label"},
            ...
            function getSeparator() { return {control : "separator"}; }
              ...
              if (layoutItem.type === 'subobject-view') {
                ...
                if (toolbar['toggle-frame'].length === 0) {
                  toolbar['toggle-frame'] =
                      [ getToggleFrameButton(selectedParent, selectedObjects) ];
                }
                ...
                if (toolbar.remove.length === 0) {
                  toolbar.remove = [ getRemoveButton(
                      selectedParent, selectionPath, selectedObjects) ];
                }
              } else if (layoutItem.type === 'telemetry-view') {
                if (toolbar['display-mode'].length === 0) {
                  toolbar['display-mode'] =
                      [ getDisplayModeMenu(selectedParent, selectedObjects) ];
                }
                if (toolbar['telemetry-value'].length === 0) {
                  toolbar['telemetry-value'] =
                      [ getTelemetryValueMenu(selectionPath, selectedObjects) ];
                }
