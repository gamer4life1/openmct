...
let openmct = {}, mockListener, testCriterionDefinition, testTelemetryObject,
    telemetryCriterion;
...
describe("The telemetry criterion", function() {
  beforeEach(() => {
    testTelemetryObject = {
      ...
      telemetry : {
        valueMetadatas : [
          {key : "value", name : "Value", hints : {range : 2}},
    ...
    openmct.telemetry = jasmine.createSpyObj('telemetry', [
      'isTelemetryObject', "subscribe", "getMetadata", "getValueFormatter"
    ]);
    ...
    openmct.time = jasmine.createSpyObj(
        'timeAPI', [ 'timeSystem', 'bounds', 'getAllTimeSystems' ]);
    ...
    telemetryCriterion =
        new TelemetryCriterion(testCriterionDefinition, openmct);

    telemetryCriterion.on('criterionResultUpdated', mockListener);
  });

  it("initializes with a telemetry objectId as string", function() {
    expect(telemetryCriterion.telemetryObjectIdAsString)
        .toEqual(testTelemetryObject.identifier.key);
  });

  it("returns a result on new data from relevant telemetry providers",
     function() {
  ...
  // it("does not return a result on new data from irrelavant telemetry
  // providers", function () {
  //     telemetryCriterion.getResult({
  //         value: 'Hello',
  //         utc: 'Hi',
