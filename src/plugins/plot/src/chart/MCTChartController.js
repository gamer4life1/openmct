...
define(
    [
      './MCTChartLineLinear', './MCTChartLineStepAfter', './MCTChartPointSet',
      './MCTChartAlarmPointSet', '../draw/DrawLoader', '../lib/eventHelpers',
      'lodash'
    ],
    function(MCTChartLineLinear, MCTChartLineStepAfter, MCTChartPointSet,
             MCTChartAlarmPointSet, DrawLoader, eventHelpers, _) {
      var MARKER_SIZE = 6.0, HIGHLIGHT_SIZE = MARKER_SIZE * 2.0;
      ...
      function MCTChartController($scope) {
        this.$onInit = () => {
          ...
          this.listenTo(this.config.series, 'remove', this.onSeriesRemove,
                        this);
          this.listenTo(this.config.yAxis, 'change:key', this.clearOffset,
                        this);
          this.listenTo(this.config.xAxis, 'change:key', this.clearOffset,
                        this);
      ...
      MCTChartController.prototype.onSeriesAdd = function(series) {
        this.listenTo(series, 'change:interpolate', this.changeInterpolate,
                      this);
        this.listenTo(series, 'change:markers', this.changeMarkers, this);
        this.listenTo(series, 'change:alarmMarkers', this.changeAlarmMarkers,
                      this);
      ...
      MCTChartController.prototype.changeInterpolate = function(mode, o,
                                                                series) {
        if (mode === o) {
          return;
        }
      ...
      MCTChartController.prototype.changeAlarmMarkers = function(mode, o,
                                                                 series) {
        if (mode === o) {
          return;
        }
      ...
      MCTChartController.prototype.clearOffset = function() {
        ...
        this.pointSets.forEach(function(pointSet) { pointSet.reset(); });
      };

      MCTChartController.prototype.setOffset = function(offsetPoint, index,
                                                        series) {
        ...
        this.offset.x = function(x) { return x - offsets.x; }.bind(this);
        this.offset.y = function(y) { return y - offsets.y; }.bind(this);
      ...
      MCTChartController.prototype.updateViewport = function() {
        ...
        var dimensions = [ xRange.max - xRange.min, yRange.max - yRange.min ],
            origin = [ this.offset.x(xRange.min), this.offset.y(yRange.min) ];
      ...
      MCTChartController.prototype.drawAlarmPoints = function(alarmSet) {
        this.drawAPI.drawLimitPoints(alarmSet.points,
      ...
      MCTChartController.prototype.drawPoints = function(chartElement) {
        this.drawAPI.drawPoints(
            chartElement.getBuffer(), chartElement.color().asRGBAArray(),
            chartElement.count, chartElement.series.get('markerSize'));
      };

      MCTChartController.prototype.drawLine = function(chartElement) {
        this.drawAPI.drawLine(chartElement.getBuffer(),
      ...
      MCTChartController.prototype.drawHighlight = function(highlight) {
        var points = new Float32Array([
          ...
          this.offset.yVal(highlight.point, highlight.series)
            ...
            color = highlight.series.get('color').asRGBAArray(), pointCount = 1;
      ...
      MCTChartController.prototype.drawRectangle = function(rect) {
        this.drawAPI.drawSquare(
            [ this.offset.x(rect.start.x), this.offset.y(rect.start.y) ],
            [ this.offset.x(rect.end.x), this.offset.y(rect.end.y) ],
            rect.color);
      };
