...
define([ './LinearScale', '../lib/eventHelpers' ], function(LinearScale,
                                                            eventHelpers) {
  /**
  ...
  MCTPlotController.prototype.initialize = function() {
    ...
    this.cursorGuideVertical =
        this.$element[0].querySelector('.js-cursor-guide--v');
    this.cursorGuideHorizontal =
        this.$element[0].querySelector('.js-cursor-guide--h');
    ...
    this.listenTo(this.$scope, 'plot:highlight:set', this.onPlotHighlightSet,
                  this);
    this.listenTo(this.$scope, 'plot:reinitializeCanvas', this.initCanvas,
                  this);
    this.listenTo(this.config.xAxis, 'change:displayRange', this.onXAxisChange,
                  this);
    this.listenTo(this.config.yAxis, 'change:displayRange', this.onYAxisChange,
                  this);
  ...
  MCTPlotController.prototype.setUpYAxisOptions = function() {
    if (this.$scope.series.length === 1) {
      ...
      this.$scope.yKeyOptions =
          metadata.valuesForHints([ 'range' ])
              .map(function(o) { return {name : o.name, key : o.key}; });
  ...
  MCTPlotController.prototype.updateCrosshairs = function($event) {
    this.cursorGuideVertical.style.left =
        ($event.clientX - this.chartElementBounds.x) + 'px';
    this.cursorGuideHorizontal.style.top =
        ($event.clientY - this.chartElementBounds.y) + 'px';
  };
  ...
  MCTPlotController.prototype.highlightValues = function(point) {
    ...
    if (!point) {
      ...
      this.$scope.series.map(function(series) { delete series.closest; });
    } else {
      this.$scope.highlights =
          this.$scope.series
              .filter(function(series) { return series.data.length > 0; })
              .map(function(series) {
                series.closest = series.nearestPoint(point);
                return {series : series, point : series.closest};
              }, this);
    }
    this.$scope.$digest();
  };
  ...
  MCTPlotController.prototype.onMouseUp = function($event) {
    ...
    this.stopListening(this.$window, 'mousemove', this.trackMousePosition,
                       this);
  };

  MCTPlotController.prototype.isMouseClick =
      function() {
    if (!this.marquee) {
      return;
    }
  ...
  MCTPlotController.prototype.endMarquee = function() {
    ...
    var marqueeDistance = Math.sqrt(Math.pow(startPixels.x - endPixels.x, 2) +
  ...
  MCTPlotController.prototype.wheelZoom = function(event) {
    ...
    if (!plotHistoryStep) {
      plotHistoryStep = {x : xDisplayRange, y : yDisplayRange};
    }
  ...
  MCTPlotController.prototype.startPan = function($event) {
    this.trackMousePosition($event);
    this.freeze();
    this.pan = {start : this.positionOverPlot};
    $event.preventDefault();
    this.trackHistory();
    return false;
  };
  ...
  MCTPlotController.prototype.updatePan = function() {
    ...
    this.config.xAxis.set('displayRange',
                          {min : xRange.min + dX, max : xRange.max + dX});
    this.config.yAxis.set('displayRange',
                          {min : yRange.min + dY, max : yRange.max + dY});
  };
  ...
  MCTPlotController.prototype.destroy = function() { this.stopListening(); };

  MCTPlotController.prototype.toggleCursorGuide = function(
      $event) { this.cursorGuide = !this.cursorGuide; };

  MCTPlotController.prototype.toggleYAxisLabel = function(label, options,
                                                          series) {
