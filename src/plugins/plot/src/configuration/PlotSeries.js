...
define([
  ...
  var PlotSeries = Model.extend({
    ...
    locateOldObject: function (oldStyleParent) {
      return oldStyleParent.useCapability("composition").then(
        function (children) {
    ...
    fetch: function (options) {
      ...
      options = _.extend(
        {},
        { size: 1000, strategy, filters: this.filters },
        options || {}
      );
      ...
      return this.openmct.telemetry.request(this.domainObject, options).then(
        function (points) {
    ...
    purgeRecordsOutsideRange: function (range) {
      ...
      if (pointsToRemove > 0) {
        if (pointsToRemove < 1000) {
          ...
          this.data
            .slice(endIndex, this.data.length)
