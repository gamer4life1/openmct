...
define([
  ...
  class TelemetryTable extends EventEmitter {
    constructor(domainObject, openmct) {
      ...
      this.buildOptionsFromConfiguration = this.buildOptionsFromConfiguration.bind(
        this
      );
    ...
    requestDataFor(telemetryObject) {
      ...
      return this.openmct.telemetry
        ...
        .then((telemetryData) => {
          ...
          let limitEvaluator = this.openmct.telemetry.limitEvaluator(
            telemetryObject
          );
    ...
    processHistoricalData(telemetryData, columnMap, keyString, limitEvaluator) {
      let telemetryRows = telemetryData.map(
        (datum) =>
          new TelemetryTableRow(datum, columnMap, keyString, limitEvaluator)
      );
    ...
    subscribeTo(telemetryObject) {
      let subscribeOptions = this.buildOptionsFromConfiguration(
        telemetryObject
      );
      let keyString = this.openmct.objects.makeKeyString(
        telemetryObject.identifier
      ...
      this.subscriptions[keyString] = this.openmct.telemetry.subscribe(
        telemetryObject,
        (datum) => {
          // Check that telemetry object has not been removed since
          // telemetry was requested.
          ...
          if (this.paused) {
            let realtimeDatum = { datum, columnMap, keyString, limitEvaluator };

            this.datumCache.push(realtimeDatum);
          } else {
            this.processRealtimeDatum(
              datum,
              columnMap,
              keyString,
              limitEvaluator
