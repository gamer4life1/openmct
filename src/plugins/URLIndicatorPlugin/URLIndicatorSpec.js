...
define(
    [ "./URLIndicator", "./URLIndicatorPlugin", "../../MCT", "zepto" ],
    function(URLIndicator, URLIndicatorPlugin, MCT, $) {
      ...
      xdescribe("The URLIndicator", function() {
        ...
        beforeEach(function() {
          ...
          spyOn($, 'ajax');
          $.ajax.and.callFake(function(options) { ajaxOptions = options; });
        });
        ...
        describe("on initialization", function() {
          describe("with default options", function() {
            ...
            it("has a default icon class if none supplied", function() {
              expect(indicatorElement.classList.contains('icon-chain-links'))
                  .toBe(true);
            });

            it("defaults to the URL if no label supplied", function() {
              expect(indicatorElement.textContent.indexOf(pluginOptions.url) >=
                     0)
                  .toBe(true);
            });
          });
          ...
          describe("with custom options", function() {
            ...
            it("uses the custom iconClass", function() {
              expect(indicatorElement.classList.contains('iconClass-checked'))
                  .toBe(true);
            });
            ...
            it("uses custom label if supplied in initialization", function() {
              expect(indicatorElement.textContent.indexOf(
                         pluginOptions.label) >= 0)
                  .toBe(true);
            });
          });
        });
        ...
        describe("when running", function() {
          ...
          it("indicates success if connection is nominal", function() {
            jasmine.clock().tick(pluginOptions.interval + 1);
            ajaxOptions.success();
            expect(indicatorElement.classList.contains('s-status-on'))
                .toBe(true);
          });

          it("indicates an error when the server cannot be reached",
             function() {
               jasmine.clock().tick(pluginOptions.interval + 1);
               ajaxOptions.error();
               expect(
